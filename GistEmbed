(function($) {
  ...
  $.fn.gist = function(callback) {
    return this.each(function() {
      ...
      loading = 'Gist Yükleniyor ' + url + (data.file ? ', file: ' + data.file : '') + '...';
        ...
        var linkTag,
          ...
          if (response && response.div) {
            ...
            if (hideFooterOption) {
              ...
              $responseDiv.find('.gist-file').css('border-bottom', '1px solid #dadada');
            }
            ...
            $elem.html('Gist Yüklenemedi ' + url);
          }
      }
      
      function errorCallBack(textStatus) {
        $elem.html('Gist Yüklenemedi ' + url + ': ' + textStatus);
      }
